...
class _MeasuredModel(BaseLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse, None]:
        ...
        if self._charged_input + estimated > INPUT_BUDGET:
...
async def run_verified_case(trial: Continuation) -> dict[str, Any]:
    ...
    try:
        ...
        def evidence_check(answer_sha256: str) -> bool:
            if trial.fixture.get("expect_abstention"):
                return False
            audit = audit_answer_evidence(trial.ledger.read(trial.task_id), required=required)
            answers = audit["answers"]
            return bool(answers and answers[-1]["status"] == "available"
                        and answers[-1].get("answer_sha256") == answer_sha256
        ...
        def runtime(settings, config, known_secrets):
            ...
            return ExecutionRuntime(
                ...
                commands=HostOracleSandbox(commands, trial.fixture["expected"],
                                           evidence_check if required else None),
            )
        ...
        request = TaskRequest(goal=trial.fixture["goal"], permitted_paths=["answer.json"],
    ...
    if hasattr(trial, "ledger"):
        ...
        if not trial.seed_checkpoint:
            ...
            result["checkpoint_exercised"] = bool(result["learning_checkpoints"])
...
async def campaign(output: Path, cases: list[str], arms: list[str], concurrency: int, sandbox_image: str | None = None) -> list[dict[str, Any]]:
            ...
            if failed.is_set():
                ...
                try:
                    async with asyncio.timeout(900):
                        trial = LearnedContinuation(root, case, arm) if case in (*LEARNED_CASES, *HELDOUT_CASES) else DevelopmentContinuation(root, case, arm)
    ...
    _atomic_write(output / "summary.json", json.dumps({"arms": summary, "promotion": "hold",
        "screen_scope": "heldout_worker_loss" if all(case in HELDOUT_CASES for case in cases) else "development_or_mixed",
...
def main() -> None:
    ...
    parser.add_argument("--cases", nargs="+", choices=(*CASES, *LEARNED_CASES, *HELDOUT_CASES), default=["routing", "missing"])
    ...
    if "no_recall" in args.arms and any(case not in HELDOUT_CASES for case in args.cases):
    ...
    manifest = {"version": "verified-continuity-v6", "model": MODEL, "reasoning": "max",
        ...
        "decisive_sources": {case: [read.model_dump() for read in decisive_sources(fixture(case))] for case in CASES}
                            | {case: fixtures[case]["source_requirements"] for case in (*LEARNED_CASES, *HELDOUT_CASES)},
        ...
        "driver_sha256": {name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                          for name in ("verified_continuity.py", "continuity.py", "continuity_cases.py", "continuity_oracle.py", "memory_audit.py", "learned_continuity.py", "heldout_continuity.py")},
        "limitations": ["learned_* and heldout_* charge model acquisition/checkpoint/acknowledgement; other cases use seeded findings", "Forced checkpoint, not natural pressure",
