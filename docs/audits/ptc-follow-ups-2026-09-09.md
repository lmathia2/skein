...
| --- | --- | --- |
| V1 Clause rows | DeepSWE requests have no acceptance criteria, so `TaskRequest` falls back to the whole goal as one row (`harness/core/models/task.py:59`). Make clause rows first-class with stable ids: decompose the goal at initialization, or adopt the model's own claim rows on the first review. | Fixture: the Koota goal yields separate rows for `Added`, `Removed`, `onAdd`, `onRemove`; the review prompt renders rows, not the 1.6k-character goal marked MISSING. |
